    ...
    <q-card style="min-width: 350px; max-width: 600px;">
      ...
      <q-card-section>
        <q-form @submit.prevent="onSave" ref="formRef">
          ...
          <q-input v-model.number="form.factor" label="Factor (to EUR)" type="number" step="0.01" required class="q-mt-sm" />
        </q-form>
      </q-card-section>
...
const form = ref({ name: "", symbol: "", factor: 1.0 });

watch(
  () => props.currency,
  (c) => {
    if (c) form.value = { ...c, factor: Number(c.factor) };
    else form.value = { name: "", symbol: "", factor: 1.0 };
  },
  { immediate: true }
);
...
const onSave = async () => {
  if (isEdit.value) {
    await store.updateCurrency(props.currency.name, form.value);
  } else {
    await store.createCurrency(form.value);
  }
  emit("saved");
};
